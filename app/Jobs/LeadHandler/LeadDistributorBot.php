...
class LeadDistributorBot implements ShouldQueue
{
    ...
    protected function get_first_nth_products_in_each_seller_class_according_to_given_category($category_id)
    {
        ...
        $products = array_filter($products,function($product) use($category_id,$time_locked_sellers_ids,&$seller_ids){
            $result = in_array($product['user_info']->id,$time_locked_sellers_ids) == false && ($product['main']->sub_category_id == $category_id) && (is_null($product['user_info']->response_rate) || $product['user_info']->response_rate >= 90);
