    ...
    <div class="form-contents bg-white col-xs-12">
      <div class="wrapper-fields">
        ...
        >
            ...
            <div class="input-wrapper">
              ...
              <select
                ...
                :class="{
                  ...
                  active:
                    fieldsData[index].itemKey && !fieldsData[index].errorMsg,
                }"
              >
          ...
          <div class="col-xs-6">
            <div class="text-input-wrapper">
              ...
              <p class="error-message">
                <span
                  v-if="fieldsData[index].errorMsg"
                  v-text="fieldsData[index].errorMsg"
                ></span>
              </p>
            </div>
          </div>
          <button
            class="remove-button"
            @click="deleteRow(fieldsData[index].itemKey, index)"
          >
      ...
      <button class="add-button" @click="AddField">
        <i class="fa fa-plus"></i> افزودن مورد
      </button>
...
export default {
    ...
    return {
      ...
      defaultFieldsOptions: [
        ...
        },
        {
          ...
          description:
            "تاییدیه های کیفی، بهداشتی و سلامت کالا موجود برای این محصول",
          itemValue: "",
          alreadySelected: false,
          selectedIndex: null,
        },
        ...
        },
        {
          ...
          name: "نوع فروش",
          description:
            "شرایط پرداخت پول در معامله طبق نظر فروشنده برای فروش این محصول",
          itemValue: "",
          alreadySelected: false,
          selectedIndex: null,
        },
  ...
  methods: {
    ...
    removeItemKeyFromList: function (itemKey, selectedIndex) {
      let selectedItem = this.defaultFieldsOptions.filter(function (el) {
        return el.name == itemKey;
      })[0];

      let index = this.defaultFieldsOptions.findIndex(
        (item) => itemKey === item.name
      );
      let tempIndex = this.defaultFieldsOptions.findIndex(
      ...
      return new Promise((resolve, reject) => {
        ...
        for (let i = 0; i < cnt; i++) {
          if (this.fieldsData[i].itemValue) {
            let itemDescription = this.getItemDescription(
        ...
        temp =
          "مقدار موجودی آماده فروش برای این محصول : " +
        ...
        temp =
          "حداقل مقدار فروش این محصول توسط فروشنده در یک معامله : " +
    ...
    itemValueValidator: function (itemValue) {
      ...
      if (
        !this.$parent.validateRegx(
          itemValue,
          /^(?!.*[(@#!%$&*)])[s\u{0600}-\u{06FF}\u{060C}\u{061B}\u{061F}\u{0640}\u{066A}\u{066B}\u{066C}\u{0E}_.-،:()A-Za-z0-9 ]+$/u
        )
      ) {
    ...
    deleteRow: function (itemKey, rowId) {
      if (itemKey !== "") {
        ...
        let myIndex = this.defaultFieldsOptions.findIndex(
          (item) => itemKey === item.name
        );
    ...
    isValidRow(index) {
      if (this.deletedRows.findIndex((item) => item == index) === -1)
        return true;
      else return false;
    },
  },
