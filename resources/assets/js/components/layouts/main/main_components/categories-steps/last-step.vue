...
<template>
  <div class="row">
    <div class="main-text">
      <ul v-if="$parent.lastStepCategories" class="form-check-wrapper">
        <li v-for="(item, index) in $parent.lastStepCategories" :key="index">
          <button
            class="default-button-list"
            @click.prevent="$parent.submitCategory(item)"
          >
            {{ item.category_name }}
          </button>
