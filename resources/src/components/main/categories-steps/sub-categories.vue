...
<template>
  <div class="row">
    <div class="main-text">
      <ul v-if="$parent.subCategoriesData" class="form-check-wrapper">
        <li v-for="(item, index) in $parent.subCategoriesData" :key="index">
          <button
            class="default-button-list"
            @click.prevent="$parent.selectedSubCategoryItem(item)"
          >
            {{ item.category_name }}
          </button>
