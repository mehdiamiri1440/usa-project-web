            ...
            <div>
                ...
                <div class="article-contents  col-sm-9 ">
                    <div class="main-article-content col-sm-7">
                        ...
                        <p>استان / شهر: <span>@{{product.main.province_name + ' - ' + product.main.city_name}}</span></p>
                        <p>مقدار موجودی: <span>@{{product.main.stock}} کیلوگرم</span></p>
                        <p>قیمت: <span>@{{product.main.min_sale_price + ' - ' + product.main.max_sale_price}} تومان</span></p>
                        <p>توضیحات: <span>@{{product.main.description}}</span></p>
                    </div>
                    <div class="image-article-content col-sm-5">
                            <popup v-if="product.photos[0]" v-for="photo in product.photos" base="{{url('storage')}}/" :img="photo.file_path"></popup>
                    </div>
                </div>
            </div>
        </article>
        <div class="loading_images  col-xs-12" v-if="products.length + 1 <= productCountInPage &&  continueToLoadProducts && products.length >= productCountInEachLoad" >
            <img src="{{asset('assets/img/gif/loading.gif')}}" style="width:200px;height:200px">
        </div>
    </section>    
    <section class="col-xs-12 " v-else-if="products.length == 0 && searchActive == true">
        <h4 class="text-center" dir="rtl">جستوجو نتیجه ای نداشت.</h4>
    </section>
    <section class="col-xs-12 " v-else-if="products.length == 0 && searchText != '' ">
        <h4 class="text-center" dir="rtl">جستوجو نتیجه ای نداشت.</h4>
    </section>
