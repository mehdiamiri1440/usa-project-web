              ...
              <table class="table table-bordered table-striped">
                  ...
                  <tbody>
                        <tr>
                            ...
                            <td class="col-xs-10">{{$comment->commenter_f_name . ' ' . $comment->commenter_l_name}} <a href="{{'/profile/'.$comment->commenter_user_name}}" target="_blank">مشاهده پروفایل</a></td>
                        </tr>
                        <tr>
                            <td class="col-xs-2"><b>کامنت گیرنده</b></td>
                            <td class="col-xs-10">{{$comment->f_name . ' ' . $comment->l_name}} <a href="{{'/profile/'.$comment->user_name}}" target="_blank">مشاهده پروفایل</a></td>
                        </tr>
                        <tr>
                            <td class="col-xs-2"><b>تاریخ ثبت</b></td>
                            <td class="col-xs-10">{{$comment->created_at}}</td>
                        </tr>
                        <tr>
                            <td class="col-xs-2"><b>امنیاز داده شده :</b></td>
                            <td class="col-xs-10">{{$comment->rating_score}}</td>
                        </tr>
                        <tr>
                            <td class="col-xs-2"><b>توضیحات</b></td>
                            <td class="col-xs-10">{{$comment->text}}</td>
                        </tr>
                  </tbody>
              </table>
            </div>
          ...
          <!-- /.box -->
                ...
                <div class="row">                
                    <div class="col-xs-3 col-xs-offset-1">
                        <button type="button" class="btn btn-success btn-block" id="{{$comment->id}}" onclick="confirmComment(event)">تایید</button>
                    </div>                 
                </div> 
            <!-- <br/> -->
...
<script>
    ...
    function confirmComment(event)
    {
        ...
        $.ajax({
            url:"{{route('admin_panel_confirm_user_comment_by_id')}}",
            data:{
        ...
        .done(function(json){
            if(json.status == true){
                window.location = '{{route('load_users_unconfirmed_comments')}}';
            }
        })
        .fail(function(xhr,status,errorThrown){
