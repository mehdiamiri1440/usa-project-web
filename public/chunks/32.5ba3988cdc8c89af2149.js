...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    filterBuyAdByCategory: function filterBuyAdByCategory() {
      self.buyAdsFilter = self.buyAds.filter(function (contact) {
        var text = self.filterCategory.split(" ");
        return text.every(function (el) {
          if (contact.first_name.indexOf(el) > -1 || contact.last_name.indexOf(el) > -1 || contact.last_name.indexOf(el) > -1 || contact.last_name.indexOf(el) > -1) {
            return true;
          } else return false;
        });
      });
