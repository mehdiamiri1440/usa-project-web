    ...
    getCurrentUser: function getCurrentUser() {
      ...
      axios.post("/user/profile_info").then(function (response) {
        if (response.data.status) {
          ...
          if (_this.currentUser.user_info) {
            if (_this.currentUser.user_info.is_seller == true) {
              _this.showRegisterRequestBox = false;
            }
          }
        }
      });
  ...
  methods: {
    ...
    stockValidator: function stockValidator(number) {
      ...
      if (this.errors.stock) {
        this.registerComponentStatistics("product-register-error", "min-sale-price", "input:" + number + " error:" + this.errors.stock);
      }
    },
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  watch: {
    stock: function stock(value) {
      ...
      this.$parent.stockValidator(value);
    }
  }
});
    ...
    _c("div", { staticClass: "form-contents col-xs-12" }, [
      !_vm.isStock
        ? _c("div", { staticClass: "text-center form-buttons-wrapper" }, [
            ...
            _vm._v(" "),
            _c("button", { staticClass: "green-button bg-red" }, [
              _vm._v("خیر")
            ])
          ])
