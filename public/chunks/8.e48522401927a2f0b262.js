      ...
      axios.post("/user/profile_info").then(function (response) {
        if (response.data.status) {
          ...
          setTimeout(function () {
            ...
            if (_this.$parent.currentUser.user_info) {
              if (_this.$parent.currentUser.user_info.is_seller == true) {
                _this.$parent.showRegisterRequestBox = false;
              }
            }
            if (_this.$parent.currentUser.user_info.is_seller) {
              _this.$parent.openChat(_this.$parent.product);
            } else {
              if (_this.isChat) {
                _this.$parent.openChat(_this.$parent.product);
              } else {
                _this.$parent.activePhoneCall(_this.isMobile);
              }
            }
          }, 100);
        }
    ...
    verifyCode: function verifyCode() {
      ...
      axios.post("/verify_code", {
        ...
        if (response.data.status === true) {
          ...
          _this3.errors.verification_code = "کد وارد شده صحیح نیست یا منقضی شده است";
