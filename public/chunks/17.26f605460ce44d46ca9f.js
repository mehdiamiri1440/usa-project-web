    ...
    feed: function feed() {
      ...
      if (!this.filterCategory) {
        ...
        axios.post("/get_related_buyAds_list_to_the_seller", {
          ...
          if (response.data.buyAds.length > 0) {
            var _self$filteredBuyAds;

            self.buyAds = (_self$filteredBuyAds = self.filteredBuyAds).push.apply(_self$filteredBuyAds, _toConsumableArray(response.data.buyAds));
            self.load = false;
          } else {
            self.continueToLoadProducts = false;
            self.load = false;
        ...
        console.log(this.buyAds.length);
      }
    },
