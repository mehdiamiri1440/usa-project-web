  ...
  methods: {
    ...
    feed: function feed() {
      ...
      if (this.searchText === "" && this.provinceId === "" && this.categoryId === "" && this.continueToLoadProducts) {
        ...
        if (this.searchText) {
          this.infiniteScrollHandler(true);
          this.$router.replace({
            name: "productList",
            query: {
    ...
    infiniteScrollHandler: function infiniteScrollHandler(isSearch) {
      ...
      if (!isSearch) {
        ...
        window.onscroll = function () {
          if (window.location.pathname.includes("product-list") && !window.location.pathname.includes("category")) {
            bottom = document.documentElement.scrollTop + window.innerHeight > document.documentElement.offsetHeight - document.documentElement.scrollTop / 2;

            var newOffset = document.documentElement.offsetHeight;
            console.log(document.documentElement.scrollTop, bottom, newOffset);
            if (bottom) {
              if (newOffset > _lastOffset + 100) {
                _lastOffset = document.documentElement.offsetHeight;
                _this.feed();
              }
            }
          }
