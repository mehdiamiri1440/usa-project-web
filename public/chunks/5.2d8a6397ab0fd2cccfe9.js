    ...
    init: function init() {
      ...
      axios.post("/user/profile_info").then(function (response) {
        ...
        if (!response.data.user_info.is_verified && _this.checkVerificationAlert(_this.$route.name)) {
          _this.verificationAlertItem = true;
        } else {
          _this.verificationAlertItem = false;
        }
  ...
  watch: {
    ...
    $route: function $route(route) {
      if (!this.$parent.currentUser.user_info.is_verified && this.checkVerificationAlert(route.name)) {
        this.verificationAlertItem = true;
      } else {
        this.verificationAlertItem = false;
      }
    },
    verificationAlertItem: function verificationAlertItem(value) {
      this.$parent.verificationAlert = value;
      console.log(this.$parent.verificationAlert);
    }
  },
