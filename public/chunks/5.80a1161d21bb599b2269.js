  ...
  methods: {
    init: function init() {
      ...
      axios.post("/user/profile_info").then(function (response) {
        ...
        _this.checkVerificationAlert(_this.$router.name);
      });
    },
    ...
    checkVerificationAlert: function checkVerificationAlert(routeName) {
      var item = disableVerificationAlertRoutes.some(function (item) {
        return item == routeName;
      });

      console.log(item);
      // for (let i = 0; i < this.disableVerificationAlertRoutes.length; i++) {
