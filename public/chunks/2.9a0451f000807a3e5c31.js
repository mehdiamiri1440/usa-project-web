    ...
    submitProduct: function submitProduct() {
      ...
      axios.post("/user/add_product", formData, {
        ...
        if (response.status === 201) {
          _this2.resetAllImages = true;

          _this2.disableSubmit = true;
          _this2.popUpMsg = _this2.getProductRegisterSuccessMessage();
          __WEBPACK_IMPORTED_MODULE_5__router_router__["b" /* eventBus */].$emit("submitSuccess", _this2.popUpMsg);
          __WEBPACK_IMPORTED_MODULE_5__router_router__["b" /* eventBus */].$emit("submiting", false);

          _this2.registerComponentStatistics("product-register", "product-registered-successfully", "product-registered-successfully");

          _this2.load = false;
          _this2.successRegisterProduct = true;
          _this2.resetComponent();
          _this2.$router.push({ name: "successRegisterProduct" });

          if (response.data.product) {
            if (response.data.product.active_package_type == 0 && !response.data.buyAds) {
              setTimeout(function () {
                _this2.$parent.is_pricing_active = true;
              }, 1000);
            }
          ...
          _this2.popUpMsg = response.data.msg;
          __WEBPACK_IMPORTED_MODULE_5__router_router__["b" /* eventBus */].$emit("submitSuccess", _this2.popUpMsg);
          __WEBPACK_IMPORTED_MODULE_5__router_router__["b" /* eventBus */].$emit("submiting", false);
          _this2.load = false;
          _this2.successRegisterProduct = true;
...
var render = function() {
  ...
  return _c("div", [
    _c("section", { staticClass: "main-content col-xs-12" }, [
      ...
      _vm._v(" "),
      !_vm.$parent.successRegisterProduct && _vm.buyAds.length > 0
