...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__register_modal_steps_register_loader__ = __webpack_require__(1007);
...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    registerUser: function registerUser() {
      ...
      this.currentStep = 7;
      this.step4.password = this.makeRandomString();
      ...
      axios.post("/user/profile_info").then(function (response) {
        if (response.data.status) {
          ...
          setTimeout(function () {
            ...
            if (_this2.$parent.currentUser.user_info) {
              if (_this2.$parent.currentUser.user_info.is_seller == true) {
                _this2.$parent.showRegisterRequestBox = false;
              }
            }
            if (_this2.$parent.currentUser.user_info.is_seller) {
              _this2.$parent.openChat(_this2.$parent.product);
            } else {
              if (_this2.isChat) {
                _this2.$parent.openChat(_this2.$parent.product);
              } else {
                _this2.$parent.activePhoneCall(_this2.isMobile);
              }
            }
          }, 100);
        }
      ...
      axios.post("/verify_code", {
        ...
        if (response.data.status === true) {
          if (response.data.redirected) {
            ...
            _this4.getCurrentUser();
          } else {
            _this4.getProvinceList();
            _this4.goToStep(3);
          }
        } else if (response.data.status === false) {
          _this4.errors.verification_code = "کد وارد شده صحیح نیست یا منقضی شده است";
          ...
          _vm.isStock
            ? _c(
                "button",
                {
                  ...
                  attrs: { disabled: _vm.isStock && _vm.$parent.errors.stock },
                  on: {
                    click: function($event) {
                      $event.preventDefault()
                      _vm.callRegisterUser()
