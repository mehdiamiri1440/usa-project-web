  ...
  methods: {
    imageValidator: function imageValidator(files) {
      ...
      if (files.length) {
        ...
        for (var i = 0; i <= files.length; i++) {
          if (files[i]) {
            if (!files[i].type || files[i].type == "" || !files[i].type == "image/png" || !files[i].type == "image/jpg" || files[i].type == "image/jpeg" || files[i].type == "text/xml" || files[i].type == "application/x-gzip") {
              errorsStatus = true;
