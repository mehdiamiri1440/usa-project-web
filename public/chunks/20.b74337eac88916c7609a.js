  ...
  methods: {
    init: function init() {
      ...
      axios.post("/get_related_buyAds_list_to_the_seller").then(function (response) {
        ...
        self.validBuyAdsCount = self.allBuyAds.map(function (item) {
          if (buyAd.has_msg || buyAd.has_phone) {
            return item;
          }
        });

        // self.validBuyAdsCount = self.validBuyAdsCount.lentgh;
        console.log(self.validBuyAdsCount);
    ...
    activePhoneCall: function activePhoneCall(buyAdUserId, buyAdId) {
      ...
      if (this.currentUser.user_info.active_pakage_type != 0) {
        ...
        swal({
          text: "برای دسترسی به شماره تماس خریداران لطفا نوع عضویت خود را ارتقا دهید.",
          icon: "warning",
          className: "custom-swal-with-cancel",
          buttons: {
            success: {
