    ...
    files: function files(_files) {
      ...
      if (_files.length) {
        ...
        for (var i = 0; i <= _files.length; i++) {
          if (_files[i]) {
            if (!_files[i].type || _files[i].type == "" || _files[i].type == "image/gif" || _files[i].type == "image/svg+xml" || _files[i].type == "application/postscript" || _files[i].type == "text/xml" || _files[i].type == "application/x-gzip") {
              errorsStatus = true;
              this.errors.images_type = "تصاویر باید فرمت معتبری باشند.";
              this.$parent.errors.images_type = "تصاویر باید فرمت معتبری باشند.";
            }

            if (_files[i].size > 5242880) {
              errorsStatus = true;
              this.errors.images_size = "حجم تصویر بالا است، باید کمتر از 5 مگابایت باشد.";
            } else if (_files[i].size < 20480) {
              errorsStatus = true;
