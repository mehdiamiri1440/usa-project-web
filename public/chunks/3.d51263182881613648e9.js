  ...
  methods: (_methods = {
    ...
    stockValidator: function stockValidator(number) {
      ...
      if (!this.errors.stock) {
        if (number.length >= 13) {
          _standardNumber = this.$parent.product.stock.substring(0, 13);
          this.$parent.product.stock = this.getNumberWithCommas(_standardNumber);
          this.stock_text = this.convertUnits(_standardNumber);
        }
        var _standardNumber = this.toLatinNumbers(number);
      }
