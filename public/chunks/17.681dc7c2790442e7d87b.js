  ...
  methods: {
    ...
    infiniteScrollHandler: function infiniteScrollHandler() {
      ...
      $(window).scroll(function () {
        if (_this.$route.name == "buyAdRequestsSeller" && !_this.filterCategory && _this.continueToLoadProducts) {
    ...
    feed: function feed() {
      ...
      axios.post("/get_related_buyAds_list_to_the_seller", data).then(function (response) {
        if (response.data.buyAds.length > 0) {
          ...
          if (_this2.filterCategory) {
            _this2.$nextTick(function () {
              _this2.filterBuyAdByCategory();
            });
            _this2.load = false;
            _this2.continueToLoadProducts = false;
          } else {
    ...
    filterBuyAdByCategory: function filterBuyAdByCategory() {
      ...
      if (this.filterCategory.id) {
        axios.post("/get_related_buyAds_list_to_the_seller", {
          ...
          _filterBuyAd = response.data.buyAds;
          self.buyAds = self.allBuyAds;
        ...
        var _filterBuyAd = this.allBuyAds;
        _filterBuyAd = _filterBuyAd.filter(function (buyAd) {
          return buyAd.category_id == _this4.filterCategory.id;
        });
        this.buyAds = _filterBuyAd;
      } else {
