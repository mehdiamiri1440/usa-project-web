  ...
  methods: {
    ...
    sendVerificationCode: function sendVerificationCode() {
      ...
      axios.post("/send_verification_code", {
        ...
        _this4.step1.sendCode = false;
        _this4.goToStep(2);

        _this4.step2.verification_code = "";
        _this4.errors.verification_code = "";

        setTimeout(function () {
          _this4.step2.reSendCode = true;
        }, 120000);

        _this4.registerComponentStatistics("Register", "send-verification-code", "verification-code-sent-to-user");
      }).catch(function (err) {
        _this4.step1.sendCode = false;

        _this4.errors.phone = err.response.data.errors.phone;

        _this4.step1.sendCode = false;

        _this4.registerComponentStatistics("Register-Error", "phone-number-verification", "error:" + _this4.errors.phone);
      });
    },
    verifyCode: function verifyCode() {
      ...
      axios.post("/verify_code", {
        ...
        _this5.step3.verifyCodeLoader = false;
        ...
        if (response.data.status === true) {
          if (response.data.redirected) {
            ...
            _this5.getProvinceList();
            _this5.goToStep(3);
          }
        } else {
          _this5.errors.verification_code = response.data.msg;

          _this5.registerComponentStatistics("Register-Error", "verification-code-wrong", "error:" + _this5.errors.verification_code);
        }
      }).catch(function (error) {
        _this5.step3.verifyCodeLoader = false;

        _this5.goToStep(2);
        _this5.errors.verification_code = "وارد کردن کد الزامی است.";

        _this5.registerComponentStatistics("Register-Error", "verification-code-empty", "error:" + _this5.errors.verification_code);
      });
    },
    ...
    getProvinceList: function getProvinceList() {
      ...
      axios.post("/location/get_location_info", { cascade_list: true }).then(function (response) {
        return _this6.step3.provinceList = response.data.provinces;
      });
    },
    ...
    handleBackKeys: function handleBackKeys() {
      ...
      $(window).on("popstate", function (e) {
        if (_this7.currentStep > 1) {
          _this7.currentStep--;
          history.pushState(null, null, window.location);
        } else {
          _this7.closeRegisterModal();
        }
      });
    },
