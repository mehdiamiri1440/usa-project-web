  ...
  methods: {
    init: function init() {
      ...
      $("#pricing-modal").on("hidden.bs.modal", function (e) {
        if (_this.getCookie("closePricingModal")) {
          var closeCount = _this.getCookie("closePricingModal");
          console.log(closeCount);
          if (closeCount < 10) {
            document.cookie = "closePricingModal=" + (parseInt(closeCount) + 1);
          }
        } else {
          _this.createCookie("closePricingModal", "true", 24 * 60); //for one day
          _this.createCookie("closePricingModalCount", 1, 30 * (24 * 60)); // for 30 day
        }
      });
      ...
      axios.post("/get_show_pricing_page_status").then(function (response) {
        ...
        if (_this.getCookie("closePricingModal") != 10 && response.data.show && window.location.pathname != "/seller/register-product") {
